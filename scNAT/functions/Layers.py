...
def Get_RNA_Features(self,embedding_dim_rna):
    X_rna = tf.compat.v1.placeholder(tf.float32, shape=[None, len(self.gene)], name='Input_RNA')
    # X_rna_input = tf.squeeze(X_rna, axis=1)
    rna_features = tf.compat.v1.layers.dense(X_rna, embedding_dim_rna, tf.nn.relu)
    
    return X_rna, rna_features
...
def Conv_Model(GO, self, trainable_embedding, kernel, use_only_seq,
    ...
    GO.mat, GO.rna_features = Get_RNA_Features(self, GO.embedding_dim_rna)
    gene_features = []
    ...
    if trainable_embedding is True:
        ...
        with tf.compat.v1.variable_scope('AA_Embedding'):
            GO.embedding_layer_seq = tf.compat.v1.get_variable(name='Embedding_Layer_Seq', shape=[23, GO.embedding_dim_aa])
    ...
    if include_RNA:
        Features = [Seq_Features,gene_features,GO.rna_features]
    else:
...
def Get_RNA_Loss(fc, embedding_dim_rna, mat,alpha=1.0):
    upsample1 = tf.compat.v1.layers.dense(fc, embedding_dim_rna, tf.nn.relu)
    upsample2 = tf.compat.v1.layers.dense(upsample1, mat.shape[1], tf.nn.relu)
    loss = tf.reduce_mean((upsample2 - mat)**2, axis = 1)
    return alpha * loss

def Get_Gene_Loss(fc,embedding_layer,X_OH,alpha=1.0):
...
def Recon_Loss(inputs,logits,alpha=1.0):
