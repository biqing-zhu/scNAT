...
def Get_DF_Data(file, aa_column_alpha=None, aa_column_beta=None,
                count_column=None,sep='\t',max_length=40,aggregate_by_aa=True,v_beta_column=None,
                d_beta_column=None,j_beta_column=None,
    ...
    with pd.option_context('mode.chained_assignment', None):
        ...
        if j_beta_column is not None:
            cols_to_keep.append(j_beta_column)
...
def save_model_data(self,saver,sess,name,get,iteration=0):
    ...
    with open(os.path.join(self.Name, 'models', 'model_type.pkl'), 'wb') as f:
        pickle.dump([name, get.name, self.use_alpha, self.use_beta,
                     ...
                     self.use_v_alpha, self.use_j_alpha, self.lb_v_beta, self.lb_d_beta, self.lb_j_beta,
                     self.lb_v_alpha, self.lb_j_alpha, self.lb,
